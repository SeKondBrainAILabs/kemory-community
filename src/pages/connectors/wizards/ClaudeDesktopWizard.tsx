...
const AGENT_NAME = 'claude-desktop-agent'
const AGENT_DESC = 'Claude Desktop persistent memory MCP agent'

export function ClaudeDesktopWizard({ onClose }: Props) {
    ...
    return (
      <>
        ...
        <ul className="space-y-3 text-sm text-content-secondary">
          ...
          <li className="flex items-start gap-2">
            <Monitor size={16} className="mt-0.5 shrink-0 text-brand-primary" />
            Config: <code className="text-xs bg-surface-secondary px-1.5 py-0.5 rounded">~/Library/Application Support/Claude/claude_desktop_config.json</code>
          </li>
        </ul>
        {existingAgent && (
      ...
      <>
        ...
        <div className="space-y-3">
          ...
          {existingAgent ? (
            ...
            <p className="text-xs text-content-tertiary">An API key will be auto-generated when you click Next.</p>
          )}
        </div>
        {regError && (
  ...
  return (
    <>
      ...
      <div className="space-y-3 max-h-64 overflow-y-auto">
        <CodeBlock label="1. Add to claude_desktop_config.json:" code={buildConfig()} />
        <CodeBlock label="2. Add memory instructions to your project CLAUDE.md:" code={claudeMdSnippet} />
      </div>

      <p className="mt-3 text-center text-xs text-content-tertiary">
        Restart Claude Desktop after saving. Memory tools will appear in every conversation.
      </p>

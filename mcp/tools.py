...
from backend.services.memory_service import (
    ...
    list_namespace_raw, get_namespace_compressed, get_namespace_summary,
...
TOOL_DEFINITIONS: list[MCPToolDefinition] = [
    ...
    MCPToolDefinition(
        ...
        description=(
            ...
            "Set use_graph=true to expand results via the Cognition OS concept graph "
            "(KMV-S8.3 — graph-augmented recall). "
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "use_graph": {
                    "type": "boolean",
                    "description": (
                        "When true, expand results via Cognition OS concept graph traversal "
                        "(KMV-S8.3). Graph-sourced items are tagged [GRAPH] in the response. "
                        "Gracefully degrades if Cognition OS is unavailable."
                    ),
                    "default": False,
                },
            },
    ...
    MCPToolDefinition(
        name="s9nmem_get_namespace_summary",
        description=(
            "Return the consolidated cross-session summary (L3.1, with L3.0 "
            "fallback) for a namespace. The summary is maintained by the "
            "compression pipeline and gives agents persistent context about a "
            "namespace without pulling every memory. Requires memory:read "
        ...
        inputSchema={
            ...
            "properties": {
                "namespace": {
                    "type": "string",
                    "description": "Namespace whose summary to return",
                },
            },
            "required": ["namespace"],
        },
    ...
    MCPToolDefinition(
        ...
        description=(
            "Tiered namespace compression. "
            "mode='raw' returns L1 raw memory dicts; "
            "mode='aaak' returns the L2 lossless dialect encoding; "
            "mode='concept' returns L3.1 LLM-synthesized concepts via core-ai-backend; "
            "mode='cognition' returns L4 concept synthesis augmented with Cognition OS "
            "graph entities and relationships (gracefully degrades to L3.1 if Cognition OS "
            "is unavailable). merge_mode='current' picks the latest position in directional "
            "sequences; 'aggregate' synthesises all positions. Requires memory:read permission."
        ),
        inputSchema={
            "type": "object",
            "properties": {
                ...
                "mode": {
                    "type": "string",
                    "enum": ["raw", "aaak", "concept", "cognition"],
...
async def handle_tool_call(
    ...
    handlers = {
        ...
        "s9nmem_get_namespace_summary": _handle_get_namespace_summary,
    }
...
async def _handle_recall_memory(
    ...
    KMV-S8.3: When use_graph=True, results are expanded via the Cognition OS
    concept graph. Graph-sourced items are clearly tagged with source_type='cognition_os'
    in the formatted output so the caller can distinguish them.
    """
    request = MemorySearchRequest(
        ...
        use_graph=bool(args.get("use_graph", False)),
    )
    ...
    # KMV-S8.3: Graph-sourced items are tagged [GRAPH] so callers can distinguish them
    lines = [f"Found {result.total} memories (showing {len(result.items)}):\n"]
    for i, item in enumerate(result.items, 1):
        source_tag = " [GRAPH]" if item.source_type == "cognition_os" else ""
        lines.append(
            f"--- Memory {i}{source_tag} ---\n"
            f"ID: {item.memory_id}\n"
...
async def _handle_list_namespaces(
    ...
    Gatekeeper-filtered: namespaces the agent has no memory:read on are
    omitted. Each entry surfaces the namespace description and the L3.1
    consolidated summary so the agent can decide where to write/search.
    """
    namespaces = await list_namespaces(user_id, db, agent_id=agent_id)
    if not namespaces:
        return MCPToolResult(
            content=[{"type": "text", "text": "No namespaces accessible to this agent."}],
        )

    lines = ["Available namespaces:\n"]
    for ns in namespaces:
        line = f"  - {ns['namespace']}: {ns['count']} memories"
        if ns.get("description"):
            line += f"\n      description: {ns['description']}"
        summary = ns.get("consolidated_summary")
        if summary:
            tier = ns.get("consolidated_summary_tier") or "L3"
            preview = summary[:240] + ("..." if len(summary) > 240 else "")
            line += f"\n      [{tier}] summary: {preview}"
        related = ns.get("related_namespaces") or []
        if related:
            related_names = ", ".join(r.get("namespace", "") for r in related[:3])
...
async def _handle_get_namespace_summary(
    args: dict,
    user_id: uuid.UUID,
    agent_id: uuid.UUID,
    db: AsyncSession,
) -> MCPToolResult:
    """Handle s9nmem_get_namespace_summary tool call.

    Returns the consolidated L3.1 summary, or falls back to the latest L3.0
    concept memory. PermissionError bubbles up to the generic handler so
    denied namespaces return the standard permission-denied response.
    """
    namespace = args["namespace"]
    payload = await get_namespace_summary(user_id, agent_id, namespace, db)
    tier = payload.get("consolidated_summary_tier") or "none"
    summary = payload.get("consolidated_summary") or ""
    if not summary:
        text = (
            f"Namespace '{namespace}' has no consolidated summary yet "
            f"(needs at least 3 memories to trigger L3.1 synthesis)."
        )
    else:
        text = (
            ...
            f"Updated: {payload.get('consolidated_summary_updated_at') or '(unknown)'}\n"
            f"---\n{summary}"
        )
...
async def _handle_get_context(
    ...
    # When the caller scoped to a single namespace, prefix the response with
    # its consolidated cross-session summary (L3.1 with L3.0 fallback). This
    # gives the agent persistent context without an extra tool call.
    summary_header = ""
    if namespace:
        try:
            summary_payload = await get_namespace_summary(
                user_id, agent_id, namespace, db, skip_gatekeeper=True,
            )
            summary_text = summary_payload.get("consolidated_summary")
            if summary_text:
                tier = summary_payload.get("consolidated_summary_tier") or "L3"
                summary_header = (
                    f"[Namespace summary — {tier}]\n{summary_text}\n\n"
                )
        except Exception:
            pass
    ...
    if synthesised:
        return MCPToolResult(
            content=[{"type": "text", "text": summary_header + synthesised}],
        )
    ...
    return MCPToolResult(
        content=[{"type": "text", "text": summary_header + "\n".join(lines)}],
    )
...
async def _handle_get_compressed(
    ...
    """Handle s9nmem_get_compressed — L1/L2/L3.1/L4 with hash-cached results. Story: KMV-S11.3"""
    ...
    if mode == "raw":
        ...
        cog_available = result.get("cognition_os_available", False)
        lines = [
            f"L4 cognition synthesis of '{args['namespace']}' "
            f"(merge_mode={result.get('merge_mode')}, "
            f"source_count={result['source_count']}, "
            f"concepts={len(concepts)}, "
            f"graph_entities={len(graph_entities)}, "
            f"cognition_os={'connected' if cog_available else 'unavailable'}, "
        ...
        if graph_entities:
            ...
            for j, e in enumerate(graph_entities, 1):
                lines.append(
                    f"\n{j}. [{e.get('entity_id', '')}] {e.get('title', '')} "
                    f"(score={e.get('score', 0):.2f})\n"
                    f"   {e.get('content', '')[:200]}"
                )
        elif not cog_available:
            lines.append("\n[Cognition OS unavailable — showing concept-only synthesis]")
        text = "\n".join(lines)
    else:  # concept
        concepts = result.get("concepts", [])
        lines = [

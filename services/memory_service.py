...
class MemoryCreate(BaseModel):
    ...
    namespace_description: Optional[str] = Field(
        None, max_length=500,
        description="Optional description of the namespace; used by the matcher to detect related namespaces and persisted on NamespacePolicy.",
    )
    allow_duplicate: bool = Field(
        default=False,
        description="If True, skip the related-namespace matcher and create the namespace as-requested even if similar ones exist.",
    )
...
class MemorySearchRequest(BaseModel):
    ...
    # F12: Filter by compression tier (L1 raw / L2 AAAK / L3.1 concept).
    ...
    try:
        ...
        if bridge.enabled:
            asyncio.create_task(bridge.publish_memory_event(
                memory_id=str(memory.memory_id),  # was memory.id (wrong attr — model PK is memory_id)
